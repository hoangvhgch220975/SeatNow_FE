  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-xl border-b border-white/20 shadow-[0_40px_40px_-15px_rgba(99,14,212,0.04)]">
      <div className="max-w-7xl mx-auto flex justify-between items-center px-8 py-4">
        ...
        {/* Center Navigation (Vietnamese comment) */}
        ...
        {/* Hành động bên phải (Thông báo & Profile) */}
        <div className="flex items-center gap-4">
          ...
          {/* Profile Dropdown (Menu người dùng) */}
              ...
              <div className="hidden lg:flex flex-col items-start pr-2">
                ...
                <div className="flex items-center gap-1">
                   ...
                   <span className="text-[10px] font-black text-slate-400 tracking-tighter uppercase">
                     {user?.loyaltyPoints || 0} {t('customer.loyalty_points')}
                   </span>
                </div>
              </div>
            </div>
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-4 w-72 bg-white/95 backdrop-blur-xl rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] border border-slate-100 p-3 z-[60] animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="px-5 py-4 mb-3 border-b border-slate-50">
                   <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest mb-1">{t('customer.authenticated_account')}</p>
                ...
                <div className="space-y-1">
                  {CUSTOMER_PROFILE_MENU.map((item) => (
                    ...
                    >
                      ...
                      <span className="text-sm font-bold tracking-tight">{getProfileLabel(item.label)}</span>
                    </Link>
                  ))}
                  
                  {/* Tích hợp Bộ chuyển đổi ngôn ngữ vào Dropdown */}
                  <div className="pt-1">
