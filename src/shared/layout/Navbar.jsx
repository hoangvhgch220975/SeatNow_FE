  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-white/70 backdrop-blur-xl border-b border-white/20 shadow-[0_40px_40px_-15px_rgba(99,14,212,0.04)]">
      <div className="flex justify-between items-center max-w-7xl mx-auto px-8 py-4">
        ...
        {/* Logo - Brand Identity (Vietnamese comment) */}
        ...
        {/* Desktop Menu (Vietnamese comment) */}
        ...
        {/* Hành động Public (Đăng nhập/Đăng ký) */}
        ...
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-t p-4 space-y-4 animate-in slide-in-from-top duration-300 shadow-xl">
          ...
          <div className="pt-4 border-t border-slate-100 flex flex-col gap-2">
             ...
             <div className="grid grid-cols-2 gap-3 mt-2">
               <Link className="text-center py-3 text-primary font-bold border border-primary/20 rounded-full" to={ROUTES.LOGIN} onClick={() => setIsMobileMenuOpen(false)}>{t('common.login')}</Link>
               <Link className="text-center py-3 bg-primary text-white rounded-full font-bold shadow-md shadow-primary/20" to={ROUTES.REGISTER} onClick={() => setIsMobileMenuOpen(false)}>{t('common.signup')}</Link>
             </div>
          </div>
        </div>
      )}
