 ...
 * @description Hiển thị thanh tiến trình trạng thái đặt bàn (Booked -> Confirmed -> Checked-in)
 * @param {string} status - Trạng thái hiện tại của booking
 */
const BookingStatusTimeline = ({ status = 'confirmed' }) => {
  // Trạng thái chuẩn hóa để so sánh
  const s = status.toLowerCase();
  
  // Xác định các mốc đã hoàn thành
  ...
  return (
    ...
    {/* Stage 1: Booked */}
      <div className="relative z-10 flex flex-col items-center gap-3">
        ...
        <span className={`text-[10px] font-black uppercase tracking-widest ${isBooked ? 'text-slate-900' : 'text-slate-400'}`}>Booked</span>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center gap-3">
        ...
        <span className={`text-[10px] font-black uppercase tracking-widest ${isConfirmed ? 'text-slate-900' : 'text-slate-400'}`}>Confirmed</span>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center gap-3">
        ...
        <span className={`text-[10px] font-black uppercase tracking-widest ${isArrived ? 'text-slate-900' : 'text-slate-400'}`}>Checked-in</span>
      </div>
    </div>
  );
};
