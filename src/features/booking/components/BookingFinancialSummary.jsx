 ...
 * @description Hiển thị tóm tắt tài chính và các ghi chú đặc biệt từ khách hàng.
 * @param {object} booking - Dữ liệu đặt bàn
 */
const BookingFinancialSummary = ({ booking }) => {
  const { notes, financial } = booking;
    ...
    <div className="space-y-8">
      ...
      <div className="bg-slate-50/50 p-8 rounded-3xl border border-slate-100 italic transition-all hover:bg-white hover:shadow-soft">
        ...
        <p className="text-slate-700 font-medium leading-relaxed">
          "{notes || "No special requests provided for this reservation."}"
        </p>
      </div>

      {/* Financial Summary */}
      <div className="bg-white p-10 rounded-3xl border-2 border-slate-100/60 shadow-soft">
        <h3 className="text-xl font-black text-slate-900 mb-8 headline tracking-tight">Financial Summary</h3>
        
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <span className="text-sm font-bold text-slate-400">Reservation Deposit</span>
            <span className="text-sm font-black text-slate-900">{financial.deposit}</span>
          </div>
          
          <div className="pt-6 border-t border-slate-100 flex justify-between items-center">
            <span className="text-lg font-black text-slate-900">Total Charged</span>
          ...
          <div className="flex items-start gap-3 mt-4 text-xs text-slate-400 font-bold bg-slate-50/50 p-4 rounded-2xl">
            ...
            <p className="leading-tight">
              The deposit will be automatically deducted from your final bill at the restaurant. 
              Please present your digital check-in code at the reception.
            </p>
          </div>
        </div>
      </div>
