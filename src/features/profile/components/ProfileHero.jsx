 ...
 * @description Thẻ Hero lớn trong bố cục Bento, chứa ảnh đại diện và thông tin chính. Hỗ trợ đa ngôn ngữ.
 */
const ProfileHero = ({ user, avatarUrl, onEdit }) => {
  ...
  return (
    <div className="lg:col-span-2 bg-white/40 backdrop-blur-sm p-10 rounded-[3rem] border-2 border-slate-200/50 shadow-soft flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group">
      {/* Media Lightbox (Review-style Viewer) (Vietnamese comment) */}
      <MediaLightbox 
      ...
      {/* Main Profile Info (Vietnamese comment) */}
      <div className="flex-1 text-center md:text-left relative z-10 pt-4">
        ...
        <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tighter leading-none italic">
          {user?.name || t('profile.hero.default_member')}<span className="text-primary not-italic">.</span>
        </h1>
        <p className="text-slate-400 mb-8 text-lg font-bold tracking-tight">{user?.email || t('profile.hero.no_email')}</p>
