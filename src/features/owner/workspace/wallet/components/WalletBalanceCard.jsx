...
import { Wallet, Clock, History, CreditCard, ArrowUpRight } from 'lucide-react';
import { formatCurrency } from '@/shared/utils/formatCurrency';

/**
 * Component hiển thị các thẻ KPI tài chính cho ví nhà hàng
 * @param {Object} data - Dữ liệu từ API balance
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="bg-primary text-white p-6 rounded-2xl relative overflow-hidden group shadow-lg shadow-primary/20"
      >
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <p className="text-3xl font-extrabold tracking-tight mt-auto">{formatCurrency(balance)}</p>
          <div className="mt-4 flex items-center gap-2 text-[10px] font-bold bg-white/10 w-fit px-2 py-1 rounded-full">
            <ArrowUpRight className="w-3 h-3" />
            {t('wallet.ready_to_withdraw')}
          </div>
        </div>
      </motion.div>
      ...
      <motion.div 
        variants={itemVariants}
        className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col group hover:border-primary/20 transition-colors"
      >
        <div className="flex items-center gap-3 mb-4">
